 ...
 */
export var DateLibrary;
(function (DateLibrary) {
    function toHuman(dateString) {
        if (!StringLibrary.isDefined(dateString)) {
            return '';
        }
        const date = new Date(dateString);
        if (!isDefined(date)) {
            console.error('Invalid date. Please ensure you are providing the correct value.');
            return dateString;
        }
    ...
    DateLibrary.timeAgo = timeAgo;
    function isDefined(date) {
        return !isNaN(date.getTime());
    }
    DateLibrary.isDefined = isDefined;
