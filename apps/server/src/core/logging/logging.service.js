...
let LoggingService = class LoggingService {
    ...
    loggerService;
    logger;
    constructor(authenticationDomainFacade, loggerService) {
    ...
    logOnStart(request) {
        ...
        const authenticationPayload = this.authenticationDomainFacade.getRequestPayload(request);
        ...
        this.logger.log(`[START] ${name} - ${email} - ${id} | ${method} ${path}`, data);
    }
    logOnStop(request) {
        const path = RequestHelper.getPath(request);
        const method = RequestHelper.getMethod(request);
